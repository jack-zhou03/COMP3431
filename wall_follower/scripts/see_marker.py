		...
		if pink_blob:
			...
			for c in ["blue", "green", "yellow"]:
				...
				if blob:
					...
					if abs(pink_x - c_x) > pink_h or abs(pink_y - c_y) > pink_h:
						# print(f'pink_x = {pink_x}, pink_y = {pink_y}, h = {pink_h}')
						continue
						# pink_y = c_y  # to allow detection when not aligned
						# pink_x = c_x
...
def get_stats(blobs, colour, node):
	...
	(numLabels, _,stats, centroids) = blobs
	if numLabels == 0:
		return None
	...
	for i in range(1, numLabels):
		...
		# node.get_logger().info(f'{colour}, height of pixels = {h}')
		if h < MIN_HEIGHT:
		...
		if area > largest:
			...
			if CALIBRATION_MODE:
				...
				node.get_logger().info(f'Measured height (h) = {h}, distance = {30.2114 * pow(h, -0.985):.2f} m')
			# distance = 36.03051 * pow(h, -1.022) # obtained experimentally
			distance = 36030.515 * pow(h, -1.022) # obtained experimentally
			node.get_logger().info(f'{colour}, distance is {distance} mm')
			...
			angle = (centre - cx) * field_of_view_h / 160 + 1.5
			if angle < 0:
				angle += 360
