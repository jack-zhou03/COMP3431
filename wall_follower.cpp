...
// 	if (near_start) {update_cmd_vel(0.0, 0.0); exit(0);} 
// 	else if (scan_data_[FRONT_LEFT] < 0.19 || scan_data_[FRONT] < 0.2 || scan_data_[FRONT_RIGHT] < 0.21) {
...
// 		update_cmd_vel(0.18, 1.12); 
// 	}
// 	else if (scan_data_[FRONT] < 0.8) {
// 		if (scan_data_[FRONT] < 0.62) {
// 			// Turn right quickly
// 			update_cmd_vel(0, -0.8); 
// 		} else {
// 			// Turn right gradually
// 			update_cmd_vel(0.16, -0.9); 
// 		}
// 	}
...
void WallFollower::update_callback()
{
	...
	*/
	...
	if (near_start) {update_cmd_vel(0.0, 0.0); exit(0);} 
	else if (scan_data_[FRONT_LEFT] < 0.2 || scan_data_[FRONT] < 0.2 || scan_data_[FRONT_RIGHT] < 0.2) {
		// Too close to something in front so reverse
		...
		// Left wall disappeared so do a harsher turn
		update_cmd_vel(LINEAR_VELOCITY, 0.23);
		
		fprintf(stderr, "Left wall not found\n");
	}
	else if (scan_data_[FRONT] < 0.3|| scan_data_[FRONT_LEFT] < 0.24 ) {
		// Turn right quickly
	...
	else if (scan_data_[FRONT_LEFT] < 0.28) {
		// Too close to wall on the left or skewed towards it 
		update_cmd_vel(LINEAR_VELOCITY, -0.6); 
		...
		// Too far from wall on the left or skewed away it 
		update_cmd_vel(LINEAR_VELOCITY, 0.38); 
